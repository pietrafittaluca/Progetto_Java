...
public class Bacheca_principale implements Bacheca
{
    ...
    @FXML private TableView<Contenuto_principale> id_table;
    @FXML private TableColumn<Contenuto_principale, String> col_nome_offerta, col_citta, col_tipo;
    @FXML private TableColumn<Contenuto_principale, Float> col_molto_positivo, col_positivo, col_neutrale, col_negativo, col_molto_negativo;
    @FXML private TableColumn<Contenuto_principale, Button> col_link, col_salva;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Città
        choice_btn_citta.getItems().add("Napoli");
        choice_btn_citta.getItems().add("Milano");
        choice_btn_citta.getItems().add("Roma");
        choice_btn_citta.getItems().add("Firenze");
    ...
    public void btn_cerca_Action(ActionEvent actionEvent)
    {
        ...
        else
        {
            ...
            ObservableList<Contenuto_principale> list = FXCollections.observableArrayList();

            try
            {
                ResultSet rs = con.createStatement().executeQuery("SELECT * FROM VIAGGIO V JOIN SENTIMENT_SCORE_TRAVEL SST ON V.LINK=SST.LINK JOIN SENTIMENT_SCORE SS ON SST.ID_SENTIMENT=SS.ID_SENTIMENT WHERE V.TIPO="+"'"+tipo+"'");

                while(rs.next())
                {
                    list.add(new Contenuto_principale(rs.getString("NOME_OFFERTA"), rs.getString("CITTA"), rs.getString("TIPO"), rs.getFloat("PERC_MOLTO_POS"), rs.getFloat("PERC_POS"), rs.getFloat("PERC_NEUTRO"), rs.getFloat("PERC_NEGATIVO"), rs.getFloat("PERC_MOLTO_NEG"), rs.getString("LINK")));
                }
            }
